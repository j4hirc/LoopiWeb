...
function renderCards(solicitudes) {
  ...
  solicitudes.forEach((solicitud) => {
    ...
    const fotoUrl = solicitud.fotoEvidencia
      ? `data:image/jpeg;base64,${solicitud.fotoEvidencia}`
                ...
                <div class="location-info" style="background: #e8f6f3; padding: 10px; border-radius: 8px; border-left: 4px solid #1abc9c;">
                    ...
                    <div style="font-size: 0.9em; color: #555; margin-top: 2px;">
                        <i class="fa-solid fa-map-location-dot"></i> ${
                          ubi.direccion || "Sin dirección registrada"
                        }
                    </div>
                </div>`;
    } else {
      infoUbicacionHTML = `
                <div class="location-info" style="background: #f0faff; padding: 10px; border-radius: 8px; border-left: 4px solid #3498db;">
                    ...
                    <div style="margin-top: 5px; font-weight: bold;">
                        ${ubi.reciclador.primer_nombre} ${
        ubi.reciclador.apellido_paterno
      }
                    </div>
                    <div style="font-size: 0.9em; color: #555; margin-top: 2px;">
                        <i class="fa-solid fa-map-pin"></i> ${
                          ubi.direccion || "Ubicación móvil"
                        }
                    </div>
                </div>`;
    }
    ...
    if (solicitud.detalles && solicitud.detalles.length > 0) {
      ...
      materialesHTML +=
        '<li style="color:#e74c3c">Sin detalles de material</li>';
    }
    materialesHTML += "</ul>";
    ...
    card.innerHTML = `
            <div class="card-image-container">
                <img src="${fotoUrl}" class="evidence-img" onclick="showImage(this.src)" alt="Evidencia">
...
async function procesarSolicitud(id, accion) {
  ...
  const confirmResult = await Swal.fire({
    title:
      accion === "APROBADO"
        ? "¿Aprobar solicitud y otorgar puntos?"
        : "¿Rechazar solicitud?",
  ...
  try {
    ...
    // Animación + eliminación de la card
    ...
    // Refresca contador / lista
    setTimeout(fetchSolicitudes, 500);
  } catch (error) {
    console.error(error);
